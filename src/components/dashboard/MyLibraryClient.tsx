...
import { ShareToAILIBModal } from './ShareToAILIBModal';
...
export function MyLibraryClient({ initialTools, categories, allTags }: MyLibraryClientProps) {
  ...
  // Share modal
  const [shareTarget, setShareTarget] = useState<LibraryTool | null>(null);
  ...
  const handleRemoved = useCallback(
    async (toolId: string) => {
      ...
      // Try actual removal — if it failed, restore
      ...
      // The LibraryToolCard already called the action, this is just a safety net
      // to restore if the server action returned an error (the card handles its own error state)
    },
    [tools]
  );

  // We also handle the case where LibraryToolCard rolls back (restore the tool)
  // by passing a restore callback
  const handleRestoreTool = useCallback((tool: LibraryTool) => {
    setTools((prev) => {
